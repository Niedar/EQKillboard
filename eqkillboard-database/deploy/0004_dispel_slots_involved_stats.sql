-- Deploy eqkillboard-database:0004_dispel_slots_involved_stats to pg
-- requires: @v3.0.0
...
ALTER TABLE killmail_involved ADD dispel_slots INTEGER NULL;
ALTER TABLE killmail_involved ADD CONSTRAINT killmail_id_attacker_id_uniq UNIQUE (killmail_id, attacker_id);

CREATE VIEW character_ranked_kill_death_involved AS
    ...
    LEFT JOIN (
        ...
        FROM (
            ...
            FROM character
            INNER JOIN killmail_involved
                ON killmail_involved.attacker_id = character.id
            INNER JOIN session_killmail
                ON session_killmail.id = killmail_involved.killmail_id
...
CREATE VIEW guild_ranked_kill_death_involved AS
    WITH session_killmail AS (
        SELECT 
            *,
            victim_id || '_' || SUM(new_session) OVER (PARTITION BY victim_id ORDER BY killed_at) AS session_id
    ...
    LEFT JOIN (
        ...
        FROM (
            ...
            FROM guild
            INNER JOIN killmail_involved
                ON killmail_involved.attacker_guild_id = guild.id
            INNER JOIN session_killmail
                ON session_killmail.id = killmail_involved.killmail_id
...
CREATE FUNCTION guild_kills(guild guild) RETURNS INTEGER AS $$
    SELECT
        COUNT(DISTINCT killmail_involved.killmail_id)::INTEGER
    FROM killmail_involved
    WHERE attacker_guild_id = guild.id
$$ LANGUAGE sql STABLE;
